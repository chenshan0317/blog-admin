 ...
 * $current_p 当前请求的页码
 * $count 总的数据条数
 * $search 默认为空，每个分页li带的搜索
 */
function pageIU($size,$current_p,$count,$search=''){
  $begin=$current_p-1;
  $end=$current_p+1;
  $allpage=intval(ceil($count/$size));
  if($begin<1){
    $begin=1;
    $end=$end<$allpage?$begin+2:$allpage;
    $current_p=1;
  }
  if($end>$allpage){
    $end=$allpage;
  ...
  if($current_p>2){
    $sear=($current_p-1).$search;
    $result_ele=sprintf("<li><a href='?p=%s'>上一页</a></li>",$sear);
    echo $result_ele;
  }else{
    $result_ele="<li><a href=''>上一页</a></li>";
    echo $result_ele;
  }
  for($i=$begin;$i<=$end;$i++){
    $acti=$i === $current_p?"active":"";
    $sear=$i.$search;
    $result_ele=sprintf("<li class='%s'><a href='?p=%s'>%d</a></li>",$acti,$sear,$i);
    echo $result_ele;
  }

  if($current_p+1<$allpage){
    $sear=($current_p+1).$search;
    $result_ele=sprintf("<li><a href='?p=%s'>下一页</a></li>",$sear);
    echo $result_ele;
  }else{
    $result_ele="<li><a href=''>下一页</a></li>";
    echo $result_ele;
  }
  ...
  <div class="main">
    ...
    <div class="container-fluid">
      ...
      <div class="page-action">
        ...
        <?php pageIU($size,$current_p,$count,$search);?>
